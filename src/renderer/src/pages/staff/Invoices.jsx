...
export default function Invoices() {
  const salesId = useSelector((state) => state.auth.user._id)
  ...
  const fetchInvoices = async () => {
    setLoading(true)
    try {
      const response = await outgoingOrderApi.getOutgoingOrderBySalesId(salesId, {
                      ...
                      return (
                        <TableRow key={invoice._id}>
                          ...
                          <TableCell className="text-xs text-right">
                            {invoice.PaymentDetails.totalAmount.toFixed(2)}
                          </TableCell>
                          <TableCell className="text-xs text-right">
                            {invoice.PaymentDetails.totalDiscountAmount > 0
                              ? `${invoice.PaymentDetails.totalDiscountAmount.toFixed(2)}`
                              : '0.00'}
                          </TableCell>
                          <TableCell className="text-xs text-right">
                            {invoice.PaymentDetails.totalTaxAmount > 0
                              ? `${invoice.PaymentDetails.totalTaxAmount.toFixed(2)}`
                              : '0.00'}
                          </TableCell>
                          {/* <TableCell className="text-xs text-right">
                            {gstType !== 'igst'
                          ...
                          <TableCell className="text-xs text-right">
                            {parseFloat(invoice.PaymentDetails.totalInvoiceValue).toFixed(2)}
                          </TableCell>
