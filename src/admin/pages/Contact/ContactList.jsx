...
const ContactList = () => {
  ...
  const fetchContacts = async (isRefresh = false) => {
    try {
      ...
      setError("");
      const response = await axios.get(`${apiUrl}/api/v1/contact`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
        },
      });
      const contactData = response.data.contacts || [];
      console.log("Raw API response for contacts:", response.data.contacts );
  ...
  const safeFilteredContacts = Array.isArray(filteredContacts)
    ? filteredContacts
    : [];
  const currentItems = safeFilteredContacts.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  const totalPages = Math.ceil(safeFilteredContacts.length / itemsPerPage);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await axios.delete(`${apiUrl}/api/v1/contact/${id}`);
      ...
      alert(
        err.response?.data?.message || "Failed to delete contact submission",
      );
    } finally {
          ...
          <div style={modalContentStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={modalFieldStyle}>
              ...
              <div style={modalValueStyle}>
                {selectedContact.contactDetails || "N/A"}
              </div>
            </div>
            ...
            <div style={modalButtonContainerStyle}>
              <button
                style={deleteButtonStyle}
                onClick={() =>
                  handleDelete(selectedContact._id || selectedContact.id)
                }
                disabled={deleteLoading}
                onMouseEnter={(e) =>
                  (e.target.style.backgroundColor = "#e53e3e")
