    ...
    >
      <div
        className={`navigation-desktop-inner rounded-[48px] px-7 flex items-center justify-around shadow-[0_4px_24px_rgba(0,0,0,0.08),0_2px_8px_rgba(0,0,0,0.04)] border border-white/20 transition-all duration-300 ${
          isScrolled
            ? "bg-[hsla(169,59%,78%,0.4)] backdrop-blur-lg"
            : "bg-[hsla(169,59%,78%,1)] backdrop-blur-md"
        }`}
        style={{ padding: "0.4rem 1rem" }}
      >
        {/* Spacer */}
        <div className="flex-1" onClick={() => navigate("/")}>
          <img src={logo} alt="Logo" className="w-32 lg:w-44 h-10 cursor-pointer" />
        </div>
        ...
        <div className="nav-icons-container flex gap-3 items-center flex-1 justify-end">
          <SearchBar
            className="nav-search-icon relative w-[30px] h-7 lg:w-[38px] lg:h-[35px] flex items-center justify-center"
            onClick={() => setIsSearchOpen(!isSearchOpen)}
            isOpen={isSearchOpen}
          />
          <CartButton
            totalCartItems={totalCartItems}
            className="nav-cart-icon w-9 h-9"
          />
          <UserProfileButton
