...
export const HeroAddImage = () => {
    ...
    if (
      ...
      const timer = setTimeout(() => {
        ...
        const baseSpeed = Math.min(
          ...uniqueMessages.map((m) => speedMap[m.speed] || 18),
        );
  ...
  return (
    <Motion.div
      ...
      className="hidden sm:block absolute bottom-0 right-0 z-10 w-full sm:max-w-[70%] md:max-w-[60%] lg:max-w-3xl"
    >
      <img
        src={heroImage}
      ...
      {!isLoading && uniqueMessages.length > 0 && (
        <div
          ref={containerRef}
          className="absolute top-1/2 sm:left-[70%] md:left-[60%] lg:left-[59%] -translate-x-1/2 -translate-y-1/2 w-full bg-transparent overflow-hidden group"
          style={{ height: "40px" }}
        >
          <div
            ref={marqueeRef}
            className="font-semibold text-xs sm:text-sm lg:text-lg tracking-[0.5px]"
            style={{
