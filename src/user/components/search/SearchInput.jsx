...
const SearchInput = ({
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        // searchService.getSuggestions returns an array of products (not { data })
        const list = Array.isArray(response) ? response : response?.data || [];
        setSuggestions(list);
        setIsOpen(Boolean(list.length));
      } catch (error) {
  ...
  const handleSuggestionClick = (product) => {
    ...
    const rid = product.rid || product.id;
    navigate(`/product/${rid}`); // Navigate directly to product details (use rid if available)
  };

  return (
    <div
      ref={wrapperRef}
      className={`relative w-[90%] md:w-full mx-auto rounded-full md:left-[7%] lg:left-[60%] ${className}`}
    >
      <form onSubmit={handleSearch} className="relative group">
        ...
        >
          <div
            className="pl-6 text-gray-400 group-focus-within:text-emerald-500 transition-colors flex items-center justify-center"
      ...
      <AnimatePresence>
        {isOpen && suggestions.length > 0 && (
          ...
          >
            <div className="py-2">
              <div
                className="px-5 py-2 text-[12px] font-bold text-gray-400 uppercase tracking-wider flex items-center justify-between"
